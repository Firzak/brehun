    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 animate-fade-in">
      ...
      <div className="relative bg-gradient-to-b from-wood to-ink border border-gold-dark/30 rounded-xl p-5 sm:p-8 w-full max-w-sm mx-auto shadow-2xl space-y-5 sm:space-y-6">
        <div className="text-center space-y-2">
          <h2 className="text-gold text-base sm:text-lg font-bold">Dernière action</h2>
          <p className="text-gold/70 text-xs sm:text-sm">
            <span className="text-gold font-semibold">{prevPlayer.name}</span> a posé{' '}
            <span className="text-gold font-semibold">{lastPlay.cardCount}</span> carte(s)
          </p>
          <p className="text-gold/50 text-[10px] sm:text-xs italic">
            en prétendant que ce sont toutes des {lastPlay.claimedFigure}
          </p>
        </div>
        ...
        <div className="text-center">
          <p className="text-gold/80 text-xs sm:text-sm mb-1">
            <span className="text-ember font-bold">{currentPlayer.name}</span>, que fais-tu ?
          </p>
        </div>
